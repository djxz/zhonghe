    ...
    <div>
        <el-form :model="formInfo" :rules="rules" ref="callbackRuleForm" label-width="110px" label-position="top">
            ...
            <el-form-item label="是否执行完毕" prop="executionCompletedFlag">
                <el-select v-model="formInfo.executionCompletedFlag" :placeholder="isEdit ? '' : '请选择是否执行完毕'" clearable style="width: 100%" :disabled="isEdit">
                    <el-option v-for="dict in dict.type.sys_yes_no" :key="dict.value" :label="dict.label" :value="dict.value"></el-option>
                </el-select>
            </el-form-item>

            <el-form-item
                v-if="formInfo.executionCompletedFlag === SYS_YES_NO.sys_no"
                label="是否因非金融机构原因导致调解协议未能执行"
            ...
            >
                <el-select v-model="formInfo.financialCauseFailureFlag" :placeholder="isEdit ? '' : '请选择是否因非金融机构原因导致调解协议未能执行'" clearable style="width: 100%" :disabled="isEdit">
                    <el-option v-for="dict in dict.type.sys_yes_no" :key="dict.value" :label="dict.label" :value="dict.value"></el-option>
                </el-select>
            </el-form-item>
