...
<script>
/** api */
import { getDisputeMediation, deleteMediationRecord, getMediationRecordExpandInfo } from '@/api/project/disputeMediation';
/** components */
import DetailForm from './detailForm.vue';
import { DM_STATUS, DEPT_TYPE } from '@/views/constant/CommonConstant';
        ...
        getList(activateId) {
            getDisputeMediation(this.workOrderId).then(async res => {
                ...
                if (this.dataList.length > 0) {
                    ...
                    this.$nextTick(() => {
                        this.$refs.detailFormRef[0].open(
                            this.row,
                            this.dataList.find(i => i.mediationRecordId.toString() === this.activate)
                        );
                    });
                } else {
                    this.tabHeaderList = [];
            ...
            try {
                ...
                if (res.code === 200 && res.data != null) {
                    return {
                        ...
                        agreedReductionAmount: res.data.agreedReductionAmount ?? null
                    };
                }
                return null;
            } catch (e) {
