...
<template>
    <div>
        <el-dialog :visible.sync="dialogVisible" :close-on-click-modal="false" width="92%" :show-close="false" class="mediation-dialog" :class="{ 'mediation-dialog--script-open': smartScriptVisible }">
            <!-- <template #title>
        <div>
                    ...
                    <el-descriptions title="双方信息" :column="2" border style="margin-left: 20px; margin-right: 20px">
                        ...
                        <el-descriptions-item label="机构代表联系方式">
                            ...
                            <el-button
                                ...
                                circle
                                size="mini"
                                v-if="row.deptContactPhone"
                                style="margin-left: 5px"
                                @click="$store.dispatch('settings/setCallInfoOpen', { open: true, workOrderId: row.workOrderId, dialNumber: row.deptContactPhone })"
                            />
                        </el-descriptions-item>
                        <el-descriptions-item label="机构业务经办人员" v-if="DEPT_TYPE.bankList.includes(this.row.deptType) || DEPT_TYPE.nonBankList.includes(this.row.deptType)">
                            {{ row.deptHandlerName }}
                        </el-descriptions-item>
                        <el-descriptions-item label="机构业务经办人员联系电话" v-if="DEPT_TYPE.bankList.includes(this.row.deptType) || DEPT_TYPE.nonBankList.includes(this.row.deptType)">
                                ...
                                <el-row class="line-row">
                                    <el-col :span="24">
                                        <el-form-item label="调解过程中发现的其他问题" prop="otherProblem">
                                            <el-input
                                                v-model="diaputeForm.otherProblem"
                                                type="textarea"
                                                placeholder="请输入调解过程中发现的其他问题"
                                                maxlength="2000"
                                                :autosize="{ minRows: 2, maxRows: 4 }"
                                                show-word-limit
                                            />
