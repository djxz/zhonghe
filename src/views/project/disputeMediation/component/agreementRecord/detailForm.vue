...
<script>
/** api */
import { updateFulfillment, saveOrUpdateFulfillmentExpand } from '@/api/project/disputeMediation';
    ...
    methods: {
        open(row, data) {
            ...
            this.formData = {
                ...
                performanceRegistrationTime: data.performanceRegistrationTime || null,
                performanceRegistrationAmount: data.performanceRegistrationAmount || null,
                performanceRegistrationReason: data.performanceRegistrationReason || null
            };
        },
        ...
        async submitForm() {
            try {
                ...
                const { performanceRegistrationReason, performanceRegistrationAmount, performanceRegistrationTime, ...restChildrenFormData } = childrenFormData;

                const res = await updateFulfillment(restChildrenFormData);
                if (res.code === 200) {
                    await saveOrUpdateFulfillmentExpand({
                        fulfillmentId: this.formData.fulfillmentId,
                        workOrderId: this.formData.workOrderId,
                        manageDeptId: this.formData.manageDeptId,
                        performanceRegistrationTime,
                        performanceRegistrationAmount,
                        performanceRegistrationReason,
